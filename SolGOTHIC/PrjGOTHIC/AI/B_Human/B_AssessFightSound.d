...
func void B_AssessFightSound()
{
	...
	{
		PrintDebugNpc(PD_ZS_FRAME,"...Монстр атакует Человека!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Человек атакует Монстра!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC сам получил удар!");
		B_FullStop(self);
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC Страж, а на Подопечного напали!");
		if(Npc_GetPermAttitude(victim,other) == ATT_FRIENDLY)
		{
			PrintDebugNpc(PD_ZS_Check,"...был просто 'friendly fire'!");
			return;
		}
		else
		{
			PrintGlobals(PD_ZS_Check);
			PrintDebugNpc(PD_ZS_Check,"...и он тоже злится или убегает!");
			B_FullStop(self);
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC принадлежит к гильдии БОСС!");
		B_SmartTurnToNpc(self,victim);
		B_AssessFighter();
		return;
	}
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Жертва в Радиусе WatchFight!");
