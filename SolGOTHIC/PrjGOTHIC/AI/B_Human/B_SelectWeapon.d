...
func void B_FillQuiver(var C_Npc slf)
{
	...
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...активное Оружие использует Стрелы!");
		addedAmmo = 20 - C_NpcHasAmmo(slf,ItAmArrow);
		CreateInvItems(slf,ItAmArrow,addedAmmo);
		PrintDebugInt(PD_ZS_DETAIL,"...добавленные Стрелы: ",addedAmmo);
	}
	else if(readiedWeapon.munition == ItAmBolt)
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...активное Оружие использует Болты!");
		addedAmmo = 20 - C_NpcHasAmmo(slf,ItAmBolt);
		CreateInvItems(slf,ItAmBolt,addedAmmo);
		PrintDebugInt(PD_ZS_DETAIL,"...добавленные Болты: ",addedAmmo);
	}
	else
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...ОШИБКА: активное Оружие не имеет Действительного типа Боеприпасов!!!");
	};
};
...
func int B_EquipAndDrawBestMeleeWeapon(var C_Npc slf)
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...заранее уберите Заклинание!");
		B_FullStop(slf);
		AI_UnreadySpell(slf);
	};
	if(Npc_HasRangedWeaponWithAmmo(slf))
	{
		PrintDebugNpc(PD_ZS_Check,"...доступно Оружие дальнего боя с Боеприпасами!");
		if(!Npc_HasReadiedRangedWeapon(slf))
		{
			PrintDebugNpc(PD_ZS_Check,"...Оружие дальнего боя еще не извлечено!");
			AI_StopAim(slf);
			B_FullStop(slf);
			AI_RemoveWeapon(slf);
	...
	if(C_NpcHasWeapon(slf,ITEM_KAT_NF))
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...еще нет Оружия ближнего боя!");
			B_FullStop(slf);
			AI_RemoveWeapon(slf);
...
func int B_EquipAndDrawBestRangedWeapon(var C_Npc slf)
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...НЕТ Оружия дальнего боя с Боеприпасами!");
	};
	return FALSE;
};
...
func void B_DrawSpell(var C_Npc slf,var int spell,var int mana)
{
	...
	if(Npc_IsInFightMode(slf,FMODE_MAGIC))
	{
		PrintDebugNpc(PD_ZS_Check,"...уже в режиме Магии!");
		if(Npc_GetActiveSpell(slf) != spell)
		{
			PrintDebugNpc(PD_ZS_Check,"...Это заклинание еще не доступно!");
			B_FullStop(slf);
			AI_UnreadySpell(slf);
		};
...
func void B_SelectWeapon(var C_Npc slf,var C_Npc oth)
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC это Маг!");
		if(C_NpcHasSpell(slf,SPL_SUMMONDEMON) && (Npc_GetDistToNpc(slf,oth) > 1200))
		{
			PrintDebugNpc(PD_ZS_Check,"...Призвать Демона!");
			Npc_PerceiveAll(slf);
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...такое уже есть!");
			};
		};
		if(C_NpcHasSpell(slf,SPL_SUMMONGOLEM) && (Npc_GetDistToNpc(slf,oth) > 1000))
		{
			PrintDebugNpc(PD_ZS_Check,"...Призвать Голема!");
			Npc_PerceiveAll(slf);
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Призвать Скелета!");
			Npc_PerceiveAll(slf);
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...уже есть другие!");
			};
		};
		if(C_NpcHasSpell(slf,SPL_ICECUBE) && !Npc_IsInState(oth,ZS_MagicFreeze))
		{
			PrintDebugNpc(PD_ZS_Check,"...Ледяная Глыба!");
		...
		else if(C_NpcHasSpell(slf,SPL_WINDFIST) && (Npc_GetDistToNpc(slf,oth) < SPL_RANGE_WINDFIST))
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...средний!");
				B_DrawSpell(slf,SPL_WINDFIST,SPL_SENDCAST_WINDFIST / 2);
			}
			else
			{
				PrintDebugNpc(PD_ZS_Check,"...маленький!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Огненный Шар!");
		...
		else if(C_NpcHasSpell(slf,SPL_FIRESTORM))
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...средняя!");
				B_DrawSpell(slf,SPL_FIRESTORM,SPL_SENDCAST_FIRESTORM / 2);
			}
			else
			{
				PrintDebugNpc(PD_ZS_Check,"...маленькая!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Стрела Молнии!");
			B_DrawSpell(slf,SPL_THUNDERBOLT,SPL_SENDCAST_THUNDERBOLT);
			return;
		}
		else if(C_NpcHasSpell(slf,SPL_PYROKINESIS))
		{
			PrintDebugNpc(PD_ZS_Check,"...Пирокинез!");
			B_DrawSpell(slf,SPL_PYROKINESIS,10);
			return;
		}
		else if(C_NpcHasSpell(slf,SPL_BREATHOFDEATH))
		{
			PrintDebugNpc(PD_ZS_Check,"...Дыхание Смерти!");
			B_DrawSpell(slf,SPL_BREATHOFDEATH,SPL_SENDCAST_BREATHOFDEATH);
			return;
		};
	};
	PrintDebugNpc(PD_ZS_Check,"...Тактика боя с использованием Оружия!");
	if((Npc_GetDistToNpc(slf,oth) < HAI_DIST_MELEE) && (slf.fight_tactic != FAI_HUMAN_RANGED))
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник в радиусе Ближнего боя, а не чистый Стрелок!");
		if(B_EquipAndDrawBestMeleeWeapon(slf))
		{
			return;
		};
		PrintDebugNpc(PD_ZS_Check,"...в Инвентаре нет оружия Ближнего боя  -> Рукопашный бой!");
		AI_ReadyMeleeWeapon(slf);
		return;
	}
	else
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник в радиусе дальнего боя или NSC чистый Стрелок!");
		...
		if(!B_FetchWeapon())
		{
			PrintDebugNpc(PD_ZS_Check,"...Оружие не найдено -> Рукопашный бой!");
			AI_ReadyMeleeWeapon(slf);
			return;
		};
	};
