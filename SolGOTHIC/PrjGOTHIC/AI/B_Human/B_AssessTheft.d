...
func void B_AssessTheft()
{
	...
	if(!Npc_IsPlayer(other))
	{
		PrintDebugNpc(PD_ZS_Check,"...другой Nsc поднял Предмет");
		return;
	};
	AI_QuickLook(self,other);
	if(Npc_CanSeeNpc(self,other))
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC видит Вора!");
		if(Hlp_IsValidItem(item) && !Hlp_IsValidNpc(victim))
		{
			PrintDebugNpc(PD_ZS_Check,"...Предмет был поднят!");
			item_ownerguild = item.ownerGuild;
			if(Npc_OwnedByNpc(item,self))
			{
				PrintDebugNpc(PD_ZS_Check,"...Предмет принадлежит NSC!");
			...
			{
				B_FullStop(self);
				PrintDebugNpc(PD_ZS_Check,"Владелец Гильдии.");
				C_LookAtNpc(self,other);
				AI_StartState(self,ZS_CatchThief,0,"");
				return;
			}
			...
			{
				B_FullStop(self);
				PrintDebugNpc(PD_ZS_Check,"Искомый Предмет");
				if(C_AmIStronger(self,other) && !Wld_DetectItem(self,ITEM_MULTI))
				{
					PrintDebugNpc(PD_ZS_Check,"Искомый Предмет и Более Мощный");
		...
		else
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...Жертва - ДРУЖЕЛЮБНАЯ/НЕЙТРАЛЬНАЯ, а Вор - ВРАЖДЕБНЫЙ");
				B_FullStop(self);
				C_LookAtNpc(self,other);
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC НЕ видит Вора!");
	};
};
