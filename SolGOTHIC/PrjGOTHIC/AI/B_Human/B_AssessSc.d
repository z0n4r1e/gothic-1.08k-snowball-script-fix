...
func void B_CheckStolenEquipment()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC носит оружие Ближнего боя NSC открыто!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...NSC не был побежден SC и еще не спрашивал об этом!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC носит оружие Дальнего боя NSC открыто!");
...
func void B_AssessSC()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC виден и находится в пределах досягаемости Диалога!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...SC в Боевом режиме!");
			B_AssessFighter();
			return;
		};
		if(!C_BodyStateContains(other,BS_SNEAK))
		{
			PrintDebugNpc(PD_ZS_Check,"...SC НЕ крадет!");
			if(Npc_GetDistToNpc(self,other) < HAI_DIST_OBSERVEINTRUDER)
			{
				PrintDebugNpc(PD_ZS_Check,"...SC в Радиусе действия ObserveIntruder!");
				B_ObserveIntruder();
				return;
			}
			else
			{
				PrintDebugNpc(PD_ZS_Check,"...SC вне Радиуса действия ObserveIntruder!");
			};
		};
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC был предупрежден стражами Прохода!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...сброс Статуса для стражей Прохода!");
