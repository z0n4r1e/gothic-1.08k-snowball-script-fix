...
func void B_MoveNpc()
{
	...
	if(other.aivar[AIV_INVINCIBLE] == FALSE)
	{
		...
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий или Заблокированный в Сценарии!!!");
			return;
		};
		if(C_NpcIsDown(other))
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий неспособен к бою!");
			return;
		};
		if(C_NpcTypeIsFriend(self,other))
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий это друг!");
			return;
		};
		if(other.aivar[AIV_INVINCIBLE])
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий находится в ZS_Talk!");
			return;
		};
		if(!C_BodyStateContains(other,BS_STAND))
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий Движется!");
			return;
		};
		if(Npc_GetAttitude(self,other) == ATT_HOSTILE)
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий враждебен!");
			B_AssessEnemy();
			return;
		}
		else if(!Npc_IsPlayer(other))
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...Блокирующий не Игрок!");
		...
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...я неважный NSC!");
			B_FullStop(self);
