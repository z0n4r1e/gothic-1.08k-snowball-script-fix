...
func void B_SmartTurnToNpc(var C_Npc slf,var C_Npc oth)
{
	...
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...не сидит и не находится на Mobsi");
		if(!Npc_CanSeeNpc(slf,oth))
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...не могу видеть Цель!");
...
func void B_WhirlAround(var C_Npc slf,var C_Npc oth)
{
	...
	if(Npc_CanSeeNpc(slf,oth))
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...МОГУ видеть Цель!");
		AI_TurnToNPC(slf,oth);
	}
	else
	{
...
func void B_RegainDroppedWeapon(var C_Npc slf)
{
	...
	if(Wld_DetectItem(slf,ITEM_KAT_NF) || Wld_DetectItem(slf,ITEM_KAT_FF))
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...NSC снова поднимает свое Оружие!");
...
func void B_RegainDroppedArmor(var C_Npc slf)
{
	...
	if(Wld_DetectItem(slf,ITEM_KAT_ARMOR))
	{
		if(!Npc_IsPlayer(slf))
		{
			PrintDebugNpc(PD_ZS_Check,"...NSC снова снимает свою Броню!");
			AI_TakeItem(slf,item);
...
func void B_GotoFP(var C_Npc slf,var string fp)
{
	...
	{
		PrintDebugString(PD_TA_CHECK,"...не на подходящей Freepoint ",fp);
		if(Wld_IsNextFPAvailable(self,fp))
		{
			PrintDebugString(PD_TA_CHECK,"Иди к Freepoint ",fp);
...
func void B_StopGotoHero()
{
	...
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...Столкновение с Игроком!");
...
func int B_CheckForImportantInfo(var C_Npc slf,var C_Npc oth)
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC не говорит!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...предоставить важную Информацию!");
			PrintDebugNpc(PD_ZS_Check,"...SC не прыгает или NSC является Охраной на проходе!");
			if(!C_BodyStateContains(oth,BS_FALL))
			{
				PrintDebugNpc(PD_ZS_Check,"...не падает!");
				if(!C_BodyStateContains(oth,BS_SWIM))
				{
					PrintDebugNpc(PD_ZS_Check,"...не плавает!");
					if(!C_BodyStateContains(oth,BS_DIVE))
					{
						PrintDebugNpc(PD_ZS_Check,"...не ныряет!");
