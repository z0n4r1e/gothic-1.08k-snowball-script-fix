...
func int C_NpcHasAmmo(var C_Npc slf,var int category)
{
	...
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...неверная Категория !!!");
		return FALSE;
	};
	count = Npc_GetInvItemBySlot(slf,INV_WEAPON,1);
	if(Hlp_IsValidItem(item) && (item.mainflag == ITEM_KAT_MUN))
	{
		PrintDebugInt(PD_ZS_DETAIL,"...найдено Количество: ",count);
		if(Hlp_GetInstanceID(item) == category)
		{
			PrintDebugString(PD_ZS_DETAIL,"...тип Боеприпасов: ",item.name);
			return count;
		};
	};
...
func int C_NpcHasWeapon(var C_Npc slf,var int category)
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...неверная Категория !!!");
		return FALSE;
	};
	Npc_GetInvItemBySlot(slf,INV_WEAPON,1);
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 1 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
		}
		else if(C_NpcHasAmmo(slf,item.munition))
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие дальнего боя с достаточным количеством Боеприпасов!");
			return TRUE;
		};
	};
	...
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 2 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
	...
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 3 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
	...
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 4 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
	...
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 5 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
	...
	if(Hlp_IsValidItem(item) && (item.mainflag == category))
	{
		PrintDebugNpc(PD_ZS_Check,"...Оружие в Слоте 6 !!!");
		if(category == ITEM_KAT_NF)
		{
			PrintDebugNpc(PD_ZS_Check,"...найдено Оружие ближнего боя!");
			return TRUE;
...
func int C_OtherIsToleratedEnemy(var C_Npc slf,var C_Npc oth)
{
	...
	if(C_NpcIsOrc(slf))
	{
		PrintDebugNpc(PD_ZS_DETAIL,"...'self' это Орк!");
		weapon = Npc_GetReadiedWeapon(oth);
		if(Hlp_IsValidItem(weapon))
		{
			weaponInstance = Hlp_GetInstanceID(weapon);
			PrintDebugString(PD_ZS_DETAIL,"...Оружие снято из 'other': ",weapon.name);
		...
		{
			weaponInstance = Hlp_GetInstanceID(weapon);
			PrintDebugString(PD_ZS_DETAIL,"...Оружие одето на 'other': ",weapon.name);
			if(weaponInstance == UluMulu)
			{
				PrintDebugNpc(PD_ZS_DETAIL,"...true!");
				return TRUE;
...
func void B_TolerateEnemy(var C_Npc slf,var C_Npc oth)
{
	...
	if(C_NpcIsOrc(slf) && (Npc_GetDistToNpc(slf,oth) > HAI_DIST_ASSESSTOLERATEDENEMY))
	{
		...
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...'self' это другой Орк!");
			if(!Npc_IsInState(slf,ZS_Upset))
			{
				PrintDebugNpc(PD_ZS_DETAIL,"...'self' еще не в ZS_FollowPC");
		...
		{
			PrintDebugNpc(PD_ZS_DETAIL,"...'self' это Орк Воин Храма!");
