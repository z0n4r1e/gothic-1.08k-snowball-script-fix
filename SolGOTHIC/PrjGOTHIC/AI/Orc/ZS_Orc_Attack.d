...
func int ZS_Orc_Attack_Loop()
{
	...
	{
		PrintDebugNpc(PD_ORC_CHECK,"Orc_Attack_Loop: ...Цель недействительна!");
		return LOOP_END;
	};
	if(C_NpcIsDown(other))
	{
		PrintDebugNpc(PD_ORC_CHECK,"Orc_Attack_Loop: Противник мертв");
		return LOOP_END;
	};
	if(Npc_GetTarget(self) && !C_NpcIsDown(other))
	{
		PrintDebugNpc(PD_ORC_LOOP,"...Цель доступна!");
		if(C_BodyStateContains(other,BS_RUN))
		{
			PrintDebugNpc(PD_ORC_LOOP,"...Цель бежит!");
			if(Npc_GetStateTime(self) > 10)
			{
				PrintDebugNpc(PD_ORC_CHECK,"...Цель преследуется слишком долго!");
