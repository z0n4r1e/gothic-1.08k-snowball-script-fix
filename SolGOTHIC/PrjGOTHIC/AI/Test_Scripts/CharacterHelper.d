...
func void B_SetHeroExp()
{
	...
	Attributes = "Повысить Уровень: Очки Обучения:";
	Attributes = ConcatStrings(Attributes,IntToString(hero.lp));
	Attributes = ConcatStrings(Attributes,"Очки Жизни:");
...
func void B_SetHeroWeapon()
{
	if(hero.level <= 5)
	{
		...
		PrintScreen("необходимо: STR 11  DEX 16",-1,-1,"font_old_20_white.tga",10);
	}
	else if(hero.level <= 10)
	{
		CreateInvItem(hero,ItMw_1H_Sword_Long_01);
		CreateInvItem(hero,ItRw_Bow_Long_02);
		PrintScreen("необходимо: STR 17  DEX 22",-1,-1,"font_old_20_white.tga",10);
	}
	else if(hero.level <= 15)
	{
		CreateInvItem(hero,ItMw_1H_Sword_Broad_01);
		CreateInvItem(hero,ItRw_Bow_Long_05);
		PrintScreen("необходимо: STR 26  DEX 28",-1,-1,"font_old_20_white.tga",10);
	}
	else if(hero.level <= 20)
	{
		...
		PrintScreen("необходимо: STR 52  DEX 34",-1,-1,"font_old_20_white.tga",10);
	}
	else if(hero.level <= 25)
	{
		...
		PrintScreen("необходимо: STR 62  DEX 40",-1,-1,"font_old_20_white.tga",10);
	}
	else if(hero.level <= 30)
	{
		...
		PrintScreen("необходимо: STR 78  DEX 50",-1,-1,"font_old_20_white.tga",10);
	};
...
instance CH_Training_STR(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "ХАРАКТЕРИСТИКА: Сила";
};
...
func void CH_Strength_Weapon()
{
	...
	{
		PrintScreen("необходимая STR уже присутствует",-1,-1,"font_old_20_white.tga",10);
	};
};
...
instance CH_Training_DEX(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "ХАРАКТЕРИСТИКА: Ловкость";
};
...
func void CH_Training_DEX_Info()
{
	...
	Info_AddChoice(CH_Training_DEX,"Ловкость (для лучшего Оружия Дальнего Боя)",CH_Dexterity_Weapon);
	if(hero.lp >= 20)
	{
...
func void CH_Dexterity_Weapon()
{
	...
	{
		PrintScreen("необходимая DEX уже присутствует",-1,-1,"font_old_20_white.tga",10);
	};
};
...
instance CH_Training_Mana(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "ХАРАКТЕРИСТИКА: Мана";
};
...
func void CH_Training_Mana_Info()
{
	...
	Info_AddChoice(CH_Training_Mana,"Мана (на все оставшиеся Очки Обучения)",CH_Mana_all);
	if(hero.lp >= 20)
	{
