...
func void StoryHelper_INFO2_Info()
{
	...
	Info_AddChoice(StoryHelper_INFO2,"II:  Получить Альманах",StoryHelper_CorKalom_BringBook_RUNNING);
	Info_AddChoice(StoryHelper_INFO2,"II:  Получить Яйца Королевы Ползунов",StoryHelper_CorKalom_BringMCQBalls_RUNNING);
	Info_AddChoice(StoryHelper_INFO2,"II:  Принести Юнитор Кор Галому",StoryHelper_YBerion_BringFocus_SUCCESS);
...
func void StoryHelper_INFO3_Info()
{
	...
	Info_AddChoice(StoryHelper_INFO3,"III: Кор Ангар - пойти к NC",StoryHelper_CorAngar_SendToNC);
	Info_AddChoice(StoryHelper_INFO3,"III: Идол Люкор - побежден",StoryHelper_BaalLukor_DEFEAT);
	Info_AddChoice(StoryHelper_INFO3,"III: Подготовка к Кладбищу Орков",StoryHelper_OrcGraveyard);
};
...
func void StoryHelper_INFO4_Info()
{
	...
	Info_AddChoice(StoryHelper_INFO4,"IV:  Найти Части для Улу-Мулу",StoryHelper_SearchForUluMulu);
	...
	Info_AddChoice(StoryHelper_INFO4,"IV:  Диего - Свободная Шахта подверглась нападению",StoryHelper_FreeMineAmbush);
	Info_AddChoice(StoryHelper_INFO4,"IV:  Сатурас - Привести Мага Огня",StoryHelper_SaturasBringFoci_SUCCESS);
};
...
func void StoryHelper_INFO5_Info()
{
	...
	Info_AddChoice(StoryHelper_INFO5,"V:  Исследовать Затопленную Башню",StoryHelper_ExploreSunkenTower);
	Info_AddChoice(StoryHelper_INFO5,"V:  Найти Уризель",StoryHelper_FoundUriziel);
	Info_AddChoice(StoryHelper_INFO5,"V:  Подготовка к Городу Орков",StoryHelper_ReadyForOrcCity);
};
