...
func int ZS_MM_Attack_Loop()
{
	...
	{
		PrintDebugNpc(PD_MST_LOOP,"...Цель доступна!");
	...
	else
	{
		...
		else
		{
			...
			if(Hlp_IsValidNpc(other) && !C_NpcIsDown(other))
			{
				PrintDebugString(PD_MST_CHECK,"...найдена новая Цель: ",other.name);
			}
			else
			{
				PrintDebugNpc(PD_MST_CHECK,"...новая Цель недоступна!");
				return LOOP_END;
			};
		};
	};
...
func int ZS_MM_Summoned_loop()
{
	...
	{
		PrintDebugNpc(PD_MST_CHECK,"...найден новый Противник");
		Npc_SetTarget(self,other);
		Npc_ClearAIQueue(self);
		AI_StartState(self,ZS_MM_Attack,0,"");
	}
