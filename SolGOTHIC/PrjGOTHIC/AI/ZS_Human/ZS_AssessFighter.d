...
func void ZS_AssessFighter()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC будет отступать!");
...
func int ZS_AssessFighter_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC на рассто€нии Ѕлижнего бо€!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC за пределами дос€гаемости!");
		return LOOP_END;
	};
...
func int ZS_AssessFighterWait_Loop()
{
	...
	if(Npc_GetDistToNpc(self,other) > HAI_DIST_ABORT_MELEE)
	{
		PrintDebugNpc(PD_ZS_Check,"...SC вне зоны Ѕлижнего бо€!");
		B_AssessRemoveWeapon();
	};
	if(Npc_GetStateTime(self) > 5)
	{
		PrintDebugNpc(PD_ZS_Check,"...¬рем€ ожидани€ истекло!");
