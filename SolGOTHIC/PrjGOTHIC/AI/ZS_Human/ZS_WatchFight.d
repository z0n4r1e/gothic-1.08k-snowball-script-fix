...
func void ZS_WatchFight()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Жертва находится дальше, чем расстояние PERC_DIST_WATCHFIGHT!");
		AI_StartState(self,ZS_GotoFight,0,"");
	};
	if(!Npc_WasInState(self,ZS_GotoFight))
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC не выходит из ZS_GotoFight!");
...
func int ZS_WatchFight_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_LOOP,"...Обходной вариант не прибытия PERC_ASSESSMURDER!");
		return LOOP_END;
	};
	if(Npc_IsInState(other,ZS_Unconscious) || Npc_IsInState(victim,ZS_Unconscious))
	{
		PrintDebugNpc(PD_ZS_LOOP,"...Обходной вариант не прибытия PERC_ASSESSDEFEAT!");
		return LOOP_END;
	};
	if(Npc_IsInState(other,ZS_Attack) || Npc_IsInState(victim,ZS_Attack))
	{
		PrintDebugNpc(PD_ZS_Check,"...борьба продолжается!");
		if((Npc_GetDistToNpc(self,other) < PERC_DIST_INTERMEDIAT) || (Npc_GetDistToNpc(self,victim) < PERC_DIST_INTERMEDIAT))
		{
			PrintDebugNpc(PD_ZS_Check,"...Бойцы все еще достаточно близко!");
			jubel = Hlp_Random(100);
			if((jubel < 5) && (Npc_GetStateTime(self) < 3))
			{
				PrintDebugNpc(PD_ZS_LOOP,"ZS_WatchFight_Loop // Бой на расстоянии 10м // одобрение");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Бойцы слишком далеко!");
			AI_TurnToNPC(self,victim);
			AI_StartState(self,ZS_GotoFight,0,"");
			return LOOP_CONTINUE;
		};
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...больше не идет борьба!");
		return LOOP_END;
	};
	AI_TurnToNPC(self,other);
	return LOOP_CONTINUE;
