...
func int ZS_AssessQuietSound_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_LOOP,"...SC обнаружен!");
		B_AssessSC();
		if((Npc_GetDistToNpc(self,other) < PERC_DIST_FLEE) && !other.aivar[AIV_INVINCIBLE])
		{
			PrintDebugNpc(PD_ZS_LOOP,"...SC в радиусе Побега!");
			Npc_ClearAIQueue(self);
			AI_StartState(self,ZS_ObserveIntruder,0,"");
		}
		else
		{
			PrintDebugNpc(PD_ZS_LOOP,"...SC вне радиуса Побега или в Разговоре!");
			return LOOP_END;
		};
	}
	else
	{
		PrintDebugNpc(PD_ZS_LOOP,"...SC все еще не обнаружен!");
		if(Npc_GetStateTime(self) > 5)
		{
			PrintDebugNpc(PD_ZS_LOOP,"...достаточно подождать 5 Секунд!");
			return LOOP_END;
		}
		else
		{
