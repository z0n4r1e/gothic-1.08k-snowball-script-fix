...
func int ZS_Attack_Loop()
{
	...
	if(C_NpcIsDown(other) || !Hlp_IsValidNpc(other))
	{
		...
		if(Npc_GetNextTarget(self))
		{
			...
			{
				PrintDebugString(PD_ZS_Check,"...найдена новая Цель: ",other.name);
			}
			else
			{
				PrintDebugNpc(PD_ZS_Check,"...Новая Цель отсутствует!");
				return LOOP_END;
			};
		}
		else
		{
			PrintDebugNpc(PD_ZS_Check,"...Новая Цель НЕ найдена!");
			return LOOP_END;
		};
	};
	if(Npc_GetStateTime(self) > 2)
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...НИ NSC ни противники не вооружены дальнобойным Оружием!");
			if((Npc_GetDistToNpc(self,other) > HAI_DIST_ABORT_MELEE) && (self.aivar[AIV_LASTHITBYRANGEDWEAPON] == FALSE))
			{
				PrintDebugNpc(PD_ZS_Check,"...Противники вне дистанции Ближнего боя, и последний Удар не был нанесен дальнобойным Оружием!");
				B_FullStop(self);
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...либо NSC либо Противники ВООРУЖЕНЫ дальнобойным Оружием!");
			if(Npc_GetDistToNpc(self,other) > HAI_DIST_ABORT_RANGED)
			{
				PrintDebugNpc(PD_ZS_Check,"...Противники вне Дальнобойной дистанции!");
				B_FullStop(self);
				return LOOP_END;
			};
		};
		...
		if(Npc_IsPlayer(other) && (C_BodyStateContains(other,BS_RUN) || C_BodyStateContains(other,BS_JUMP)) && !Npc_IsInFightMode(self,FMODE_FAR) && !Npc_IsInFightMode(self,FMODE_MAGIC) && (C_GetAttackReason(self) != AIV_AR_INTRUDER))
		{
			...
			{
				PrintDebugNpc(PD_ZS_Check,"...Противник бежит слишком долго!");
				B_FullStop(self);
				if(C_NpcIsHuman(other))
				{
					B_AssessAndMemorize(NEWS_DEFEAT,NEWS_SOURCE_WITNESS,self,self,other);
...
func void ZS_Attack_End()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Цель без сознания/в магическом сне, и NSC не чистый стрелок!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Враждебный/Нарушитель будет уничтожен");
			B_FullStop(self);
			PrintGlobals(PD_ZS_Check);
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Не враждебный");
	...
	if(!C_NpcIsBoss(self) && C_NpcIsHuman(other) && (Npc_IsInState(other,ZS_Unconscious) || Npc_IsDead(other) || Npc_IsInState(other,ZS_MagicSleep)))
	{
		...
		{
			PrintDebugNpc(PD_ZS_Check,"...Условия грабежа выполнены!");
...
func void B_CombatRemoveWeapon()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Причина нападения: Оружие не убрано!");
		if(self.attribute[ATR_HITPOINTS] == self.attribute[ATR_HITPOINTS_MAX])
		{
			PrintDebugNpc(PD_ZS_Check,"...NSC не пострадал!");
			B_FullStop(self);
			B_ResetTempAttitude(self);
...
func void B_CombatAssessEnterRoom()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC покинул Комнату NSC и еще ничего не украл!");
		if(self.attribute[ATR_HITPOINTS] == self.attribute[ATR_HITPOINTS_MAX])
		{
			PrintDebugNpc(PD_ZS_Check,"...NSC не пострадал!");
			B_FullStop(self);
