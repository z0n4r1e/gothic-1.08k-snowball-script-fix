...
func void ZS_ObserveIntruder()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Нарушитель в режиме Боя!");
		B_AssessFighter();
		return;
	}
	else if(C_BodyStateContains(other,BS_SNEAK))
	{
		PrintDebugNpc(PD_ZS_Check,"...Нарушитель крадется!");
		B_ObserveSuspect();
		return;
	}
	else
	{
		PrintDebugNpc(PD_ZS_Check,"...Нарушитель НЕ в режиме Боя и не крадется!");
