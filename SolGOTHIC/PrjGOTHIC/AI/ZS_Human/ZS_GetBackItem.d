...
func int ZS_GetBackItem_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_LOOP,"ZS_GetBackItem_Loop // после 3 секунд Атака");
...
func void B_GetBackItem_Seen()
{
	...
	{
		PrintDebugNpc(PD_ZS_FRAME,"B_GetBackItem_Seen хочу это");
...
func void B_GetBackItem_Given()
{
	...
	if(self.aivar[AIV_WANTEDITEM] == Hlp_GetInstanceID(item))
	{
		PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given // хочу эту вещь, потому что она моя");
		Npc_ClearAIQueue(self);
	...
	else
	{
		if(C_WantItem())
		{
			PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given // хочу этого, потому что это достойно");
			Npc_ClearAIQueue(self);
			Npc_GiveItem(other,item,self);
		...
		{
			PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given не хочу этого дерьма");
			B_Say(self,other,"$YOUWANNAFOOLME");
			if(C_AmIStronger(self,other))
			{
				PrintDebugNpc(PD_ZS_FRAME,"ZS_GetBackItem_Given не хочу этого дерьма // сильнее");
				Npc_ClearAIQueue(self);
				Npc_SetTarget(self,other);
				AI_StartState(self,ZS_Attack,0,"");
			};
