...
func void ZS_AssessWarn()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Предупреждение от Подопечного, в чью Комнату входили/выходили!");
		...
		{
			PrintDebugNpc(PD_ZS_Check,"Нарушитель уже наказан");
		};
	}
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Предупреждение от Стража ворот, Проход которых был нарушен");
		B_WhirlAround(self,hero);
		B_DrawWeapon(self,other);
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Предупреждение о Враге!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Игрок крадется, и меня предупредили");
		Npc_SetTarget(self,other);
		Npc_GetTarget(self);
...
func void ZS_AssessWarn_End()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn Завершение // Потеря Свободы");
		if((Npc_GetTempAttitude(self,other) == ATT_HOSTILE) || (Npc_GetPermAttitude(self,other) == ATT_HOSTILE))
		{
			PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn Завершение // Потеря Свободы // ВРАЖДЕБНЫЙ");
			Npc_SetTarget(self,other);
			Npc_GetTarget(self);
			AI_StartState(self,ZS_AssessEnemy,0,"");
			return;
		...
		{
			PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn Завершение // Потеря Свободы //Не ВРАЖДЕБНЫЙ Друг убит");
			Npc_GetTarget(self);
			AI_StartState(self,ZS_AssessEnemy,0,"");
		}
		else
		{
			PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn Завершение// Потеря Свободы // Не ВРАЖДЕБНЫЙ");
			B_AssessSC();
			return;
		};
		return;
	...
	{
		PrintDebugNpc(PD_ZS_Check,"ZS_AssessWarn  Завершение// Не Вижу");
		return;
	};
};
