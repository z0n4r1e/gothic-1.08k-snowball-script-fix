...
func int ZS_Flee_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник больше не актуален!");
		return LOOP_END;
	}
	else if(Npc_GetDistToNpc(self,other) > HAI_DIST_ABORT_FLEE)
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник достаточно далеко -> Прервать попытку побега!");
		return LOOP_END;
	}
	else if(C_NpcIsDown(other))
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник больше не способен к бою!");
		return LOOP_END;
	}
	else
	{
		PrintDebugNpc(PD_ZS_Check,"...Противник все еще слишком близко!");
