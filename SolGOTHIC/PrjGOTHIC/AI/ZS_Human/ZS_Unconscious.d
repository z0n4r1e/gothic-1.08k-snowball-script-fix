...
func void ZS_Unconscious()
{
	...
	self.aivar[AIV_PLUNDERED] = 0;
	if(Npc_CanSeeNpc(self,other) && (self.guild < GIL_SEPERATOR_ORC))
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC видит Преступника!");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...побежден SC или членом Группы!");
		B_UnconciousXP();
		self.aivar[AIV_WASDEFEATEDBYSC] = 1;
	};
	if(Npc_IsPlayer(self))
	{
		PrintDebugNpc(PD_ZS_Check,"...SC побежден!");
		other.aivar[AIV_HASDEFEATEDSC] = 1;
	};
};
...
func int ZS_Unconscious_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Конец Цикла");
		return LOOP_END;
	};
	AI_Wait(self,1);
...
func void ZS_Unconscious_End()
{
	...
	{
		PrintDebugNpc(PD_ZS_FRAME,"ZS_Unconscious_End: Я, Орк -> временно-ВРАЖДЕБНЫЙ");
		AI_Standup(self);
		Npc_PerceiveAll(self);
		if(Wld_DetectItem(self,ITEM_KAT_NF) || Wld_DetectItem(self,ITEM_KAT_FF))
		{
			PrintDebugNpc(PD_ZS_Check,"ZS_Unconscious_End: Я, Орк -> Обнажить Оружие");
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...NSC все еще видит Нападающего, и тот находится в Сознании!");
