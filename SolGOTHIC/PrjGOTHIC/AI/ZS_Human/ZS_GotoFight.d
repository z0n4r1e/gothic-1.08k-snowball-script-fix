...
func int ZS_GotoFight_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...Нападающий или Жертва недействительны!");
		return LOOP_END;
	};
	if(Npc_IsDead(other) || Npc_IsDead(victim))
	{
		PrintDebugNpc(PD_ZS_Check,"...Нападающий или Жертва мертвы!");
		return LOOP_END;
	};
	if(Npc_IsInState(other,ZS_Unconscious) || Npc_IsInState(victim,ZS_Unconscious))
	{
		PrintDebugNpc(PD_ZS_Check,"...Нападающий или Жертва без сознания!");
		return LOOP_END;
	};
	if(Npc_IsInState(other,ZS_Flee) || Npc_IsInState(victim,ZS_Flee))
	{
		PrintDebugNpc(PD_ZS_Check,"...Нападающий или Жертва в ZS_Flee!");
		return LOOP_END;
	};
	AI_Wait(self,1);
