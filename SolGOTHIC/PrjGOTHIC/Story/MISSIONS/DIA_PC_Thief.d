...
{
	AI_Output(hero,self,"Info_Diego_Rules_15_00");	//Что нужно знать, для того чтобы присоединиться к вашему Лагерю?
...
instance Info_Diego_Teach(C_Info)
{
	...
	permanent = 1;
	description = "А ты можешь меня чему-нибудь научить?";
};
...
instance Info_Diego_IanPassword(C_Info)
{
	...
	permanent = 0;
	description = "...почему ты думаешь, что Ян отдаст его МНЕ?";
};
...
func void Info_Diego_OCSTORY_Info()
{
	...
	AI_Output(self,hero,"Info_Diego_OCSTORY_11_08");	//Если быть честным, это единственный шанс Гомеза удержать власть в своих руках.
...
instance Info_Diego_OCWARNNC(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "...Я должен предупредить Новый Лагерь!";
};
...
instance Info_Diego_OCFAVOR(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "Считай, что все уже сделано!";
};
