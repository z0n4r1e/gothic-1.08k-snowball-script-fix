...
func void DIA_BaalTaran_Lehre_Info()
{
	...
	AI_Output(self,other,"DIA_BaalTaran_Lehre_05_04");	//Знай, что те, кто присоединятся к Спящему, обретут бессмертную душу, неверных же ждет беспощадная кара.
...
instance DIA_BaalTaran_IntoCastle(C_Info)
{
	...
	permanent = 1;
	description = "Я должен попасть в замок.";
};
...
func void DIA_BaalTaran_IntoCastle_Info()
{
	...
	AI_Output(self,other,"DIA_BaalTaran_IntoCastle_05_03");	//Если бы ты был одним из гонцов Кор Галома, то без труда проник бы в замок. Но прежде ты должен стать одним из членов Братства.
	...
	Info_AddChoice(DIA_BaalTaran_IntoCastle,"У меня есть письмо для Верховного Мага Круга Огня.",DIA_BaalTaran_IntoCastle_Letter);
};
...
func void DIA_BaalTaran_IntoCastle_Letter()
{
	...
	AI_Output(self,other,"DIA_BaalTaran_IntoCastle_Letter_05_01");	//О-о! Откуда оно у тебя? Тебе его дали маги, прежде чем забросить сюда, верно?
...
func void DIA_BaalTaran_IntoCastle_Mage()
{
	...
	AI_Output(self,other,"DIA_BaalTaran_IntoCastle_Mage_05_01");	//О-о! Знаешь, они никогда не покидают замок. Тебе действительно придется как-то попасть внутрь.
	AI_Output(self,other,"DIA_BaalTaran_IntoCastle_Mage_05_02");	//Но что тебе от него нужно? Магические руны? Или эликсир?
	Info_ClearChoices(DIA_BaalTaran_IntoCastle);
	Info_AddChoice(DIA_BaalTaran_IntoCastle,"Тебя это не касается.",DIA_BaalTaran_IntoCastle_ForgetIt);
	Info_AddChoice(DIA_BaalTaran_IntoCastle,"Угадал.",DIA_BaalTaran_IntoCastle_Exactly);
};
