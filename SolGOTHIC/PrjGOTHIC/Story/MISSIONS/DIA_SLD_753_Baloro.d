...
func void DIA_SLD_753_Baloro_Wasmeinstdu_Info()
{
	...
	AI_Output(other,self,"DIA_SLD_753_Baloro_Wasmeinstdu_Info_15_03");	//В самом деле? Ха-ммм, может быть, ты и прав.
...
func void DIA_SLD_753_Baloro_Worumgehts_ja()
{
	...
	AI_Output(self,other,"DIA_SLD_753_Baloro_Worumgehts_ja_08_05");	//Поверь мне, ты не пожалеешь. Я уже говорил тебе, что с этим оружием можно победить любого врага?
	Info_ClearChoices(DIA_SLD_753_Baloro_Worumgehts);
	Info_AddChoice(DIA_SLD_753_Baloro_Worumgehts,"Ладно! Посмотрим, что я смогу для тебя сделать!",DIA_SLD_753_Baloro_Worumgehts_jaklar);
	Info_AddChoice(DIA_SLD_753_Baloro_Worumgehts,"Нет, нет! Забудь! Мне это не нужно!",DIA_SLD_753_Baloro_Exit_Info);
};
...
func void DIA_SLD_753_Baloro_habsdabei_Info()
{
	...
	AI_Output(self,other,"DIA_SLD_753_Baloro_habsdabei_Info_08_06");	//Но это не важно. Теперь у нас есть все для хорошей вечеринки! Большое спасибо! Может быть, мы еще когда-нибудь встретимся. Ха-ха-ха-ха-ха-ха-ха!
...
instance DIA_SLD_753_Baloro_Exit(C_Info)
{
	...
	important = 0;
	permanent = 1;
	description = "Извини, нет времени поболтать с тобой еще!";
};
...
instance DIA_SLD_753_Baloro_Angebotdochannehmen(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "...Может, вернемся к твоему предложению?";
};
