...
func void DIA_Horatio_Hello_BeCool()
{
	...
	AI_Output(self,other,"DIA_Horatio_Hello_BeCool_09_01");	//А-а. Ну тогда ладно. Трудно угадать заранее. Каждый день здесь проходит толпа народа, и каждый думает, что мир вращется именно вокруг него. 
	Info_ClearChoices(DIA_Horatio_Hello);
};
...
instance DIA_Horatio_PleaseTeachSTR(C_Info)
{
	...
	permanent = 0;
	description = "Как мне стать таким же сильным, как ты?";
};
...
func void DIA_Horatio_PleaseTeachSTR_Attack()
{
	...
	AI_Output(self,other,"DIA_Horatio_PleaseTeachSTR_Attack_09_01");	//Прежде чем это случится, ты станешь одним из этих ублюдков... Нет, я не буду учить тебя ради этого.
	Info_ClearChoices(DIA_Horatio_PleaseTeachSTR);
};
