...
instance Info_Xardas_SLEEPER(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "...'Спящий' на самом деле является злым демоном.";
};
...
instance Info_Xardas_EVENT(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "Не скажешь? Но почему?";
};
...
func void Info_Xardas_LOADSWORD01_Info()
{
	...
	AI_Output(self,other,"Info_Xardas_LOADSWORD01_14_02");	//Встречаются упоминания о том, что меч этот без труда пронзает любой доспех и разрушает самые мощные защитные заклинания.
...
func void Info_Xardas_OREARMOR_Info()
{
	...
	{
		AI_Output(self,other,"Info_Xardas_OREARMOR_14_01");	//Я вижу на тебе железный доспех.
	}
	else
	{
		AI_Output(self,other,"Info_Xardas_OREARMOR_14_02");	//Я вижу, ты нашел железный доспех.
	};
...
instance Info_Xardas_FORMULA(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "Ты подготовил заклинание..?";
};
...
{
	AI_Output(other,self,"Info_Xardas_ALTRUNE_15_01");	//Но я же маг! Мне нелегко будет управляться с мечом!
...
instance Info_Xardas_MAKERUNE(C_Info)
{
	...
	important = 0;
	permanent = 0;
	description = "...маг сможет воспользоваться этим мечом.";
};
