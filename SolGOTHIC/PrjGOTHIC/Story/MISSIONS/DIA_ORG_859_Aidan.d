...
func void Org_859_Aidan_Creatures_Info()
{
	...
	if((Knows_GetTeeth == FALSE) || (Knows_GetFur == FALSE) || (Knows_GetClaws == FALSE) || (Knows_GetHide == FALSE))
	{
		...
		{
			Info_AddChoice(Org_859_Aidan_Creatures,"Удаление Клыков (1 Очко Обучения, 50 Руды)",Org_859_Aidan_Creatures_Zahn);
		};
		if(Knows_GetFur == FALSE)
		{
			Info_AddChoice(Org_859_Aidan_Creatures,"Снятие Шкуры (1 Очко Обучения, 100 Руды)",Org_859_Aidan_Creatures_Fell);
		};
		if(Knows_GetClaws == FALSE)
		{
			Info_AddChoice(Org_859_Aidan_Creatures,"Удаление Когтей (1 Очко Обучения, 50 Руды)",Org_859_Aidan_Creatures_Kralle);
		};
		if(Knows_GetHide == FALSE)
		{
			Info_AddChoice(Org_859_Aidan_Creatures,"Снятие Кожы (1 Очко Обучения, 100 Руды)",Org_859_Aidan_Creatures_Haut);
		};
	}
	else
	{
...
func void Org_859_Aidan_Creatures_Zahn()
{
	...
	if(Npc_HasItems(other,ItMiNugget) >= 50)
	{
		if(other.lp >= 1)
		{
			...
			PrintScreen("Изучено: Удаление Клыков",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
			...
			B_LogEntry(GE_AnimalTrophies,"Знание об вырезании клыков у волков, орочьих гончих, глорхов, мракорисов, ищеек и кусачей.");
		}
		else
		{
...
func void Org_859_Aidan_Creatures_Fell()
{
	...
	if(Npc_HasItems(other,ItMiNugget) >= 100)
	{
		if(other.lp >= 1)
		{
			...
			PrintScreen("Изучено: Снятие Шкуры",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
...
func void Org_859_Aidan_Creatures_Kralle()
{
	if(Npc_HasItems(other,ItMiNugget) >= 50)
	{
		if(other.lp >= 1)
		{
			other.lp = other.lp - 1;
			B_GiveInvItems(other,self,ItMiNugget,50);
			PrintScreen("Изучено: Удаление Когтей",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
...
func void Org_859_Aidan_Creatures_Haut()
{
	if(Npc_HasItems(other,ItMiNugget) >= 100)
	{
		if(other.lp >= 1)
		{
			other.lp = other.lp - 1;
			B_GiveInvItems(other,self,ItMiNugget,100);
			PrintScreen("Изучено: Снятие Кожы",-1,-1,"FONT_OLD_20_WHITE.TGA",2);
