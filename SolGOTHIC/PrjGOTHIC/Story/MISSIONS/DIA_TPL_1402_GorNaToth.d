...
func void DIA_GorNaToth_AngarTalked_Info()
{
	...
	Info_AddChoice(DIA_GorNaToth_AngarTalked,"...что ты не достоин носить доспехи Стража.",DIA_GorNaToth_AngarTalked_Unworthy);
	Info_AddChoice(DIA_GorNaToth_AngarTalked,"...что ему не нравятся болотожоры.",DIA_GorNaToth_AngarTalked_Shark);
	Info_AddChoice(DIA_GorNaToth_AngarTalked,"...чтобы я возвращался, как только стану Стражем.",DIA_GorNaToth_AngarTalked_Normal);
};
...
func void DIA_GorNaToth_AngarTalked_Shark()
{
	...
	AI_Output(self,other,"DIA_GorNaToth_AngarTalked_Shark_11_01");	//Что?.. Меня ожидает СВЯЩЕННАЯ миссия!
...
func void TPL_1402_GorNaToth_GETSTUFF_Info()
{
	...
	AI_Output(self,other,"TPL_1402_GorNaToth_GETSTUFF_Info_11_02");	//Я счастлив, что мне выпала честь передать эти доспехи человеку, который нашел кладку ползунов.
	AI_Output(self,other,"TPL_1402_GorNaToth_GETSTUFF_Info_11_03");	//Пусть эти доспех хранит тебя, как хранит наше Братство Спящий!
...
func void TPL_1402_GorNaToth_TRAIN_Info()
{
	if(log_gornatothfight == FALSE)
	{
		...
		log_gornatothfight = 1;
	};
	AI_Output(other,self,"TPL_1402_GorNaToth_TRAIN_Info_15_00");	//Я хотел бы потренироваться с одноручным оружием.
	if(B_GiveSkill(hero,NPC_TALENT_1H,1,LPCOST_TALENT_1H_1))
	{
		AI_Output(self,other,"TPL_1402_GorNaToth_TRAIN_11_01");	//Хорошая мысль! Но прежде чем заняться техникой, тебе нужно научиться правильно держать оружие.
