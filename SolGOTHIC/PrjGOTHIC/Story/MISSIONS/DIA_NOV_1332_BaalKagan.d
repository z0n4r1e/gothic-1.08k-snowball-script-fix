...
instance DIA_BaalKagan_GimmeKraut(C_Info)
{
	...
	permanent = 0;
	description = "Ладно, давай мне болотник.";
};
...
func void DIA_BaalKagan_SUCCESS_Info()
{
	...
	if(NC_Joints_verteilt >= 8)
	{
		...
		Info_AddChoice(DIA_BaalKagan_SUCCESS,"Я возьму 100 кусков руды.",DIA_BaalKagan_SUCCESS_Erz);
		if(Npc_GetTrueGuild(hero) == GIL_None)
		{
			Info_AddChoice(DIA_BaalKagan_SUCCESS,"Помоги мне стать членом Братства.",DIA_BaalKagan_SUCCESS_Join);
		};
		Info_AddChoice(DIA_BaalKagan_SUCCESS,"Я возьму магические свитки.",DIA_BaalKagan_SUCCESS_WhatSpells);
