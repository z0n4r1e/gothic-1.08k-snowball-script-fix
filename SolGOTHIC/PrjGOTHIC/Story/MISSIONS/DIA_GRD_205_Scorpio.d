...
func void DIA_Scorpio_Hello_Info()
{
	...
	{
		Info_AddChoice(DIA_Scorpio_Hello,"Я собираюсь встретиться с Гомезом.",DIA_Scorpio_Hello_Join);
	};
};
...
func void GRD_205_Scorpio_CROSSBOW_Info()
{
	...
	AI_Output(self,other,"GRD_205_Scorpio_CROSSBOW_Info_13_02");	//Если у тебя есть руда, я научу тебя пользоваться арбалетом. Но сначала заплати две сотни. 
	if((log_scorpiocrossbow == FALSE) && (Kapitel <= 4))
	{
		Log_CreateTopic(GE_TeacherOC,LOG_NOTE);
		B_LogEntry(GE_TeacherOC,"Скорпио может научить меня использовать арбалет.");
		log_scorpiocrossbow = 1;
	}
