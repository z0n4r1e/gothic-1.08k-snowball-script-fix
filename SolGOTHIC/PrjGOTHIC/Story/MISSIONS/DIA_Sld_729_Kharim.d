...
func void Info_Kharim_Charge_Insult()
{
	...
	Info_ClearChoices(Info_Kharim_Charge);
	Info_AddChoice(Info_Kharim_Charge,"Ты здесь..., чтобы... поцеловать ... Гомеза в задницу!",Info_Kharim_Charge_Insult_GomezAss);
	Info_AddChoice(Info_Kharim_Charge,"...твоя мать была похожа на овцу...",Info_Kharim_Charge_Insult_Goats);
...
{
	AI_Output(other,self,"Info_Kharim_Charge_Insult_Stupid_15_00");	//Подумай-ка вот о чем... Хотя нет, не могу придумать ничего такого, что бы ты смог понять.
...
func void Info_Kharim_InArena_Info()
{
	...
	{
		AI_Output(self,other,"SVM_9_LetsForgetOurLittleFight");	//Я бы предложил просто забыть об этом...
		AI_StopProcessInfos(self);
		Npc_ExchangeRoutine(self,"START");
	};
};
