...
func int mc_oldmine_entrance()
{
	...
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...управляемо");
		return TRUE;
	};
	return FALSE;
};
...
func int mc_monasteryruin_gate()
{
	...
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...не вскрывалось");
		AI_UseMob(hero,"VWHEEL",1);
		AI_UseMob(hero,"VWHEEL",-1);
		MonasteryRuin_GateOpen = 1;
		return TRUE;
	}
	else
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...уже открывалось");
		return TRUE;
	};
	return FALSE;
};
...
func int mc_ogy_gate()
{
	...
	if(CorAngar_GotoOGY)
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...закрыто");
		return TRUE;
	}
	else
	{
		PrintDebugNpc(PD_ITEM_MOBSI,"...открыто");
		G_PrintScreen(_STR_MESSAGE_WHEEL_STUCKS);
		return FALSE;
	};
	return FALSE;
...
func int MC_OrcCity_Sunctum_OuterGate()
{
	...
	OrcCity_Sanctum_OuterGateTried = 1;
	if((Kapitel >= 5) && Npc_HasItems(hero,ItMi_Stuff_Idol_Sleeper_01) && (OrcCity_Sanctum_OuterGateOpen == FALSE))
	{
		...
		AI_UseMob(hero,"LEVER",-1);
		OrcCity_Sanctum_OuterGateOpen = 1;
		return TRUE;
	}
	else
	{
