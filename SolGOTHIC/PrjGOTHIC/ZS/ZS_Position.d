...
func int ZS_FriendlyAttack_Loop()
{
	...
	if(Npc_GetDistToNpc(self,other) > 1000)
	{
		...
		{
			PrintDebugNpc(PD_ZS_FRAME,"...у SC есть оружие Дальнего боя или Магия!");
			AI_SetWalkMode(self,NPC_RUN);
			AI_GotoNpc(self,other);
		}
		else
		{
			PrintDebugNpc(PD_ZS_FRAME,"...у SC есть оружие Ближнего боя");
			B_Say(self,other,"$LETSFORGETOURLITTLEFIGHT");
			return LOOP_END;
		};
	};
	...
	{
		PrintDebugNpc(PD_ZS_FRAME,"...у SC больше нет Оружия в Руке!");
		B_Say(self,other,"$LETSFORGETOURLITTLEFIGHT");
		return LOOP_END;
	};
...
func void B_CheckDistToPlayer()
{
	...
	{
		PrintDebugNpc(PD_ZS_Check,"...SC слишком далеко!");
		Npc_ClearAIQueue(self);
		AI_Standup(self);
		AI_TurnToNPC(self,other);
