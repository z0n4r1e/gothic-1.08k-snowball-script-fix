...
{
	text[0] = "Боевые Умения / Критический Шанс:";
	posx = STAT_B_X1;
...
{
	text[0] = "Магические Умения:";
	posx = STAT_B_X1;
	posy = STAT_TAL_Y + (STAT_DY * 12);
	fontname = STAT_FONT_TITLE;
	flags = flags & ~IT_SELECTABLE;
};
...
{
	text[0] = "Ремесленные Умения:";
	posx = STAT_B_X1;
	posy = STAT_TAL_Y + (STAT_DY * 17);
	fontname = STAT_FONT_TITLE;
	flags = flags & ~IT_SELECTABLE;
};
